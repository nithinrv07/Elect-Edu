...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        // ===== 6. INTERACTIVE QUIZ WITH INPUT VALIDATION =====
        const quizData = [
            {
            ...
            },
            {
                ...
                correct: 2,
                explanation: "EVM stands for Electronic Voting Machine, used since 1998 in Indian elections."
            },
            {
                ...
                correct: 1,
                explanation: "The Lok Sabha has 543 directly elected members, plus 2 appointed members from the Anglo-Indian community."
            }
        ];
        ...
        function loadQuestion() {
            ...
            try {
                ...
                questionText.textContent = sanitizeHTML(qData.question);
                if (progressText) progressText.textContent = `${currentQuestionIndex + 1} / ${quizData.length}`;
                
                qData.options.forEach((opt, index) => {
                    const btn = document.createElement('button');
                    btn.classList.add('option-btn');
                    btn.type = 'button';
                    btn.textContent = sanitizeHTML(opt);
                    btn.setAttribute('aria-label', `Option ${index + 1}: ${opt}`);
                    btn.addEventListener('click', () => selectOption(index, btn));
                    optionsContainer.appendChild(btn);
                });

                Logger.log('Question loaded:', currentQuestionIndex + 1);
            } catch (error) {
                Logger.error('Load question error', error);
            }
        }
        ...
        function selectOption(selectedIndex, btnElement) {
            try {
                ...
                Logger.log('Option selected:', selectedIndex, 'Score:', score);
            } catch (error) {
        ...
        if (nextBtn) {
            nextBtn.addEventListener('click', () => {
                try {
                    ...
                    Logger.error('Next button error', error);
                }
            });
        }
            ...
            try {
                ...
                quizBody.innerHTML = `
                    <div class="text-center">
                        ...
                        <p style="font-size: 1.2rem; margin-bottom: 0.5rem;">You scored <strong>${score} out of ${quizData.length}</strong></p>
                        <p style="font-size: 1rem; color: #475569;">Performance: ${percentage}%</p>
        ...
        Logger.log('Quiz initialized');
    } catch (error) {
        Logger.error('Quiz initialization failed', error);
    }
